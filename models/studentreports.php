<?php
	...
	{ 
		...
		function getDetails($id='') {
			$sql = "Select sr.*, st.referenceno, t.name as teacher, st.name as student, y.name as year, r.name as term, l.name as level, s.name as stage, g.name as grade, c.name as class, i.description as improvement, d.description as discipline from studentreports as sr
					inner join enrollments as e on e.id = sr.enrollid
					inner join students as st on st.id = e.studentid
					inner join teachers as t on t.id = sr.teacherid
					inner join years as y on y.id = sr.yearid
