...
<div>
	<span class="place-right"> <a class="button" href="#" onclick="openWindow('?module=reportmasters&action=partner_add')"> Add </a> </span>
...
<div>
	<table class="table table-border cell-border row-hover" border="0" data-role="dataTable">
		...
		<tbody>
	<?php foreach($partners as $id=>$R) { ?>
		<tr>
			<td nowrap style="width:120px">
				<a href="#" onclick="openWindow('?module=reportmasters&action=partner_edit&id=<?=$R['id']?>')" <?=displayHint('Edit')?> ><span class="mif-pencil"></a>
				<? if ($R['status'] == 1) { ?>
					<a href="?module=reportmasters&action=partner_status_change&id=<?=$R['id']?>&status=0" <?=displayHint('Deactivate')?>><span class="mif-cross sp_icon fg-red"></a>
				<? } else { ?>
